...
function HeaderMorpion({currentPlayer,resetGrid,sauvegarderDernierePartie , quitter}) {
    const handleClick = () => {
        resetGrid();
    ...
    return (
        <div className={"flex justify-between items-center p-3.5"}>
            ...
            <div>
                ...
                <button
                    className={"px-2 py-1 rounded-lg bg-red-500 ml-2 border-b-4 border-red-700 transition-all duration-150 ease-in-out transform hover:scale-105 hover:shadow-lg"}
                    onClick={quitter}
                >

                    <FontAwesomeIcon className={"mt-0.5"} icon={faArrowRightFromBracket } size="lg" />
                </button>
            </div>
        </div>
    )
